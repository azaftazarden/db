<html>
	...
	<body bgcolor="white">
		<?php
			...
			$result = pg_exec($link, "SELECT SUM(preco_queijo) FROM queijo WHERE id NOT IN (SELECT id FROM compra_queijo)");
    ...
    <!--coloque aqui as tabelas esperadas na ordem-->
		<table border="1">
			<tr>
				<th>valor total de queijo em estoque</th>
			</tr>
			...
			<?php
				...
				for($ri = 0; $ri < $numrows; $ri++) {
					...
					echo " <td>", $row["valor total de queijo em estoque"], "</td>
					</tr>
					";
				}
				pg_close($link);
